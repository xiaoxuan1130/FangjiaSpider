...
proxy_pool=settings.OPPOOL


class liangjiaSpider(scrapy.Spider):
    name="lianjia"
    ...
    def start_requests(self):
        ...
        for url in self.start_urls:
            ...
            yield Request(url,self.parse,meta={'proxy':proxy_addr})
        ...
        if url in self.start_urls:
            ...
            for i in range(1,totalPage):
                ...
                for j in url1:
                    url2 +=j+''
                proxy_addr=random.choice(proxy_pool)
                yield Request(url2, callback=self.parse,meta={'proxy':proxy_addr})
        else:
